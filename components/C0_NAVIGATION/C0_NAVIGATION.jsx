...
const C0_NAVIGATION = ({ expo_push_notif_token }) => {
  const app_version = "v 3.0.0";
  const db_version_path =
    "/DB2_BENBY_MERCH_APP/TBL_MAINTAINABLE/APP_VERSIONS/3-0-0/VALUE";
  ...
  const get_current_location = async () => {
    ...
    try {
      // let location = await Location.getCurrentPositionAsync({});
      let location = await Location.getCurrentPositionAsync({
        accuracy: Location.Accuracy.Balanced, // Or .Low for even faster results
      });
      // set_current_location({
